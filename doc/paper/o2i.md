...
![O2I Frameworkarchitektur](<../images/O2I Frameworkarchitektur.png>){#fig:o2i-framework-architecture width=90%}
...
![O2I Orientierung](<../images/O2I Orientierung.png>){#fig:o2i-orientation-view width=50%}
...
![O2I Strategiebestandteile](<../images/O2I Semantics - Strategy Constituents.png>){#fig:o2i-strategy-constituents-view}
...
Diagnose, strategische Absicht und Guiding Policy bilden jeweils einen zusammenhängenden Bestandteil. Sie können mehrere Aussagen zur Herausforderung, mehrere angestrebte Beiträge beziehungsweise mehrere leitende Prinzipien umfassen.[^strategy-constituent-content] Ihre Inhalte müssen gemeinsam eine kohärente Wegentscheidung tragen.

[^strategy-constituent-content]: *Autorenableitung in Anlehnung an Rumelt (2011) und Porter (1996)*: O2I unterscheidet die Einheit eines Strategiebestandteils von der Anzahl seiner Inhalte. Maßgeblich ist deren fachlicher Zusammenhang innerhalb der Strategie.
...
![O2I Situation](<../images/O2I Semantics - Situation.png>){#fig:o2i-situation-view}
...
![O2I Situationsverankerung](<../images/O2I Semantics - Situation Anchoring.png>){#fig:o2i-situation-anchoring-view}
...
Die maschinenprüfbare Spezifikation in `spec/lib/core/` bindet den geschlossenen semantischen Vertrag an typisierte Struktur- und Semantikauswertungen. Der ArchiMate-Profilvertrag, der native Adapter und die Operation-Komposition bleiben davon getrennte Eigentümer ihrer jeweiligen Übersetzung und Ausführung. Das Metamodell bleibt technologieunabhängig; Haskell-Typen, Module und opake Auswertungsergebnisse sind technische Realisierungsentscheidungen und keine zusätzliche O2I-Fachsemantik.
...
![O2I Kontextmodell](<../images/O2I Semantics - Context.png>){#fig:o2i-context-view}
...
![O2I Primitives-Modell](<../images/O2I Semantics - Primitives.png>){#fig:o2i-primitives-view width=75%}
...
![O2I Nachweisfolge](<../images/O2I Nachweisfolge.png>){#fig:o2i-evidence-sequence width=65%}
...
![O2I Modellzustand](<../images/O2I Modellzustand.png>){#fig:o2i-model-state width=72%}
...
**`Strategy`: Mindestinhalt:** kein ergänzender Mindestinhalt des Modellbeweises; fachlich genau eine vollständige Formulierung mit jeweils mindestens einem gelisteten Diagnosis-`Driver`, Intent-`Objective`, Guiding-Policy-`Principle`, einer `Action` und einem `Key Result` **Relationsevidenz:** im optionalen Strategy-Assessment universell für die gelisteten Inhalte: Jedes Intent-Objective besitzt Vision-Ausrichtung, Diagnosebegründung und Erfolgsbezug; jeder Diagnosis-Driver begründet mindestens ein Intent-Objective, jede Action wird durch mindestens ein Guiding-Policy-Principle geführt und trägt zu mindestens einem Key Result bei, und jedes Key Result substantiiert mindestens ein Intent-Objective.
...
Jede Strategy-Instanz besitzt genau eine vollständige Formulierung. Sie umfasst Geltungsbereich, strategische Verankerung, abgeleitete Leitplanken, Diagnose, strategische Absicht, Guiding Policy, Positionierung, Trade-offs, kohärente Handlungsfestlegungen, strategische Erfolgsbezüge und Fit-Begründung. Jeder strategische Erfolgsbezug wird durch ein `Key Result @ Strategy` modelliert. Sämtliche Textfelder müssen nichtleer sein. Diagnose, Absicht, Guiding Policy, Actions und Key Results werden jeweils durch eine nichtleere Menge innerhalb ihrer Rolle eindeutiger Primitive-Referenzen derselben Strategy-Instanz gebunden; ihre Reihenfolge trägt keine Semantik. Die gelisteten Primitives, ihre Kontextualisierungen und die verwendeten Relationszeugen müssen `Asserted` sein.

Die Formulierung verlangt folgende relationale Abdeckung:

- Jedes Intent-Objective wird durch mindestens ein `Objective @ Vision` ausgerichtet, durch mindestens einen gelisteten Diagnosis-Driver begründet und durch mindestens ein gelistetes Key Result substantiiert.
- Jeder gelistete Diagnosis-Driver begründet mindestens ein gelistetes Intent-Objective.
- Jede gelistete Action wird durch mindestens ein gelistetes Guiding-Policy-Principle geführt und trägt zu mindestens einem gelisteten Key Result bei.
- Jedes gelistete Key Result substantiiert mindestens ein gelistetes Intent-Objective.

Diese Pflichten verlangen passende Relationszeugen je bezeichnetem Inhalt, keine vollständige Vernetzung aller Inhalte. Die gemeinsame Handlungslogik der Guiding-Policy-Principles bleibt fachlich zu begründen; eine eigene Action-Relation für jedes Principle ist keine zusätzliche Pflicht.
...
Der maschinenlesbare Profilvertrag in `spec/ctr/archimate/profile.json` ist die exakte Quelle der konkreten ArchiMate-Syntax. Er definiert sämtliche Syntaxträgerabbildungen, Relationsabbildungen, Metadaten und Kardinalitäten; die ausführbaren Compiler- und Prüfschritte konsumieren diesen Vertrag unmittelbar.
...
![O2I ArchiMate-Syntaxträger](<../images/O2I Syntax - Carriers.png>){#fig:o2i-syntax-carriers-view width=90%}

\clearpage

![O2I ArchiMate-Relationsrepräsentationen](<../images/O2I Syntax - Relations.png>){#fig:o2i-syntax-relations-view width=95%}
...
- `spec/ctr/archimate/profile.json` bestimmt die exakte konkrete Syntax.
...
\begin{center}
\vspace*{\fill}
\includegraphics[width=\linewidth,height=0.86\textheight,keepaspectratio]{../images/O2I Layered Cake.png}
