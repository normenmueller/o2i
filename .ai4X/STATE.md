...
- Batch `#35` attempt 1 candidate `f36647d65a60eaad1ea5403aa0f0dfdac47a19ca` implements the typed collective fan-in Core. Core build and all Core tests pass with warnings as errors, Haddock reports 100% coverage for every public facade, package metadata and formatting pass, and the public-repository boundary is clean. Independent review rejects the candidate: registry dispatch still repeats family scans, work metrics omit source reads, provenance equality and witness observability are incomplete, and duplicate claim identity has competing diagnostic owners. The repository-wide Haskell gate reaches the scheduled Batch `#36` Inspection exhaustiveness work and is not a Batch `#35` completion condition.
- The Product Owner controls Issue activation, ArchiMate model edits, and pushes.

# Next Action

Correct the rejected Batch `#35` design through one closed registry routing fold, truthful work accounting, exact observable provenance binding, and one canonical claim-identity diagnostic owner; then verify and review a new exact candidate.
...
- Candidate revision: `f36647d65a60eaad1ea5403aa0f0dfdac47a19ca`
