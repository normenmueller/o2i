...
module O2I.Inspection.Diagnostic.Internal
  ( InspectionStage(..)
  , DiagnosticSeverity(..)
...
-- | Why externally supplied diagnostic-code text is inadmissible.
data DiagnosticCodeError =
  DiagnosticCodeMissingO2IPrefix
  deriving (Eq, Ord, Show)

-- | Validate a fully qualified code against the report schema namespace.
mkDiagnosticCode :: Text -> Either DiagnosticCodeError DiagnosticCode
mkDiagnosticCode value
  | "o2i." `Text.isPrefixOf` value = Right (DiagnosticCode value)
  | otherwise = Left DiagnosticCodeMissingO2IPrefix

-- | Construct an O2I-namespaced code from its adapter-owned suffix.
o2iDiagnosticCode :: Text -> DiagnosticCode
o2iDiagnosticCode = DiagnosticCode . ("o2i." <>)
...
-- | Stage-free projection target of one adapter-owned defect constructor.
...
-- | Describe an adapter defect without choosing its report stage or identity.
diagnosticSpec ::
     DiagnosticCode
  -> DiagnosticSeverity
  -> DiagnosticDisposition
  ...
  -> DiagnosticSpec
diagnosticSpec code severity disposition message subjects dataFields =
  DiagnosticSpec
    { specCode = code
    , specSeverity = severity
    , specDisposition = disposition
    , specMessage = message
...
diagnosticFromLocated stage specification located =
  diagnosticFromSpec
    stage
    [locatedAt located]
    (specification (locatedValue located))
...
coreSpec :: Text -> Text -> [DiagnosticSubject] -> DiagnosticSpec
coreSpec code message subjects =
  DiagnosticSpec
