...
-- | Context-content completeness of structurally valid O2I graphs.
--
-- This validation stage establishes global Context minima and complete,
-- coherent Strategy formulations before aggregate model semantics may be
-- assessed.
module O2I.Validation.Semantics.Context
  ( StrategyAnchoring(..)
  ...
  , contextAssessmentStatus
  , assessedContextSemantics
  , contextAssessmentInvariantErrors
  , contextAssessmentCandidatePropositions
  , contextAssessmentElaborations
  , contextAssessmentElaboration
  , contextGraph
...
data CandidateContextProposition
  = CandidateContextNode RawNode
    -- ^ Proposed element declaration.
  | CandidateContextEdge RawEdge
    -- ^ Proposed relation declaration.
  | CandidateContextStrategyFormulation RawNodeId
...
-- semantics.
data ContextSemantics = ContextSemantics
  { contextSemanticsGraph :: WellFormedGraph -- ^ Structurally valid graph.
  , contextSemanticsStrategies :: Map RawNodeId StrategyFormulation
...
data ContextAssessment = ContextAssessment
  ...
  , assessedContextElaborations :: Map RawNodeId Elaboration
  , assessedContextStatus :: ContextAssessmentStatus
  }
...
validateContextSemantics ::
     WellFormedGraph
  -> [RawStrategyFormulation]
  -> Validation (NonEmpty ModelInvariantError) ContextSemantics
validateContextSemantics graph rawFormulations =
  case NonEmpty.nonEmpty errors of
    Just failures -> Failure failures
    Nothing ->
      Success
        ContextSemantics
          { contextSemanticsGraph = graph
...
-- | Assess explicit claim commitments without weakening Context validation.
--
...
assessContextSemantics structure formulationClaims =
  ContextAssessment
    { assessedContextErrors = errors
    , assessedContextCandidates = candidates
    , assessedContextElaborations = elaborations
    , assessedContextStatus = status
    }
  where
    ...
    candidates =
      map graphCandidate (structuralCandidatePropositions structure)
        ++ [ CandidateContextStrategyFormulation
             (rawFormulationStrategy (claimedProposition formulation))
           | formulation <- formulationClaims
           , claimCommitment formulation == Candidate
           ]
    validation = validateContextSemantics graph assertedFormulations
    allErrors =
      case validation of
...
-- | Read the closed Context-content assessment state.
...
contextAssessmentInvariantErrors = assessedContextErrors

-- | Read candidate propositions excluded from validated Context semantics.
contextAssessmentCandidatePropositions ::
     ContextAssessment -> [CandidateContextProposition]
contextAssessmentCandidatePropositions = assessedContextCandidates

-- | Read every derived Context elaboration in the assessed boundary.
contextAssessmentElaborations :: ContextAssessment -> Map RawNodeId Elaboration
contextAssessmentElaborations = assessedContextElaborations
...
contextAssessmentElaboration ::
     ContextAssessment -> RawNodeId -> Maybe Elaboration
contextAssessmentElaboration assessment identifier =
  Map.lookup identifier (assessedContextElaborations assessment)

graphCandidate :: CandidateGraphProposition -> CandidateContextProposition
graphCandidate proposition =
  case proposition of
    CandidateNodeProposition node -> CandidateContextNode node
    CandidateEdgeProposition edge -> CandidateContextEdge edge
...
-- | Access the structurally well-formed graph underlying Context semantics.
contextGraph :: ContextSemantics -> WellFormedGraph
contextGraph = contextSemanticsGraph

-- | Access the complete Strategy formulations indexed by Strategy context.
contextStrategyFormulations ::
     ContextSemantics -> Map RawNodeId StrategyFormulation
contextStrategyFormulations = contextSemanticsStrategies
...
lookupContextSemanticsRef semantic = lookupContextRef (contextGraph semantic)
...
qualifyingStrategiesInContext ::
     ContextSemantics -> ContextRef 'Need -> [ContextRef 'Strategy]
qualifyingStrategiesInContext semantic need =
  [ mkContextRef strategy
  | strategy <- contextNodesOf graph Strategy
  , hasRelation graph strategy qualifiesNeed needIdentifier
  , Just formulation <-
      [Map.lookup strategy (contextStrategyFormulations semantic)]
  , let keyResults =
          NonEmpty.toList
