module ValidationCollectiveOpaqueConstructors where

import qualified O2I.Validation as Validation

forgedCollectiveStrategyRealization :: Validation.CollectiveStrategyRealization
forgedCollectiveStrategyRealization =
  Validation.CollectiveStrategyRealization
    undefined
    undefined
    undefined
    undefined
    undefined

forgedCandidateCollectiveStrategyRealization ::
     Validation.CandidateCollectiveStrategyRealization
forgedCandidateCollectiveStrategyRealization =
  Validation.CandidateCollectiveStrategyRealization undefined undefined
