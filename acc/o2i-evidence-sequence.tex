...
\node[phase, above=0.22cm of visible] {
  AUSGANGSPUNKT \quad SEMANTISCH GÜLTIGES MODELL
};
...
\draw[flow] (visible) -- node[gate] {
  \textbf{Capability: Bedarfsqualifikation}\\[-1pt]
  {\color{omuted}Vorlage formal prüfen; fachlich annehmen}\\[-1pt]
  {\color{omuted}und Qualifikationsrelationen modellieren}
} (relevant);

\node[state, below=1.72cm of relevant] (traceable) {
  \textbf{Vollständiger Wirkungstrace}\\[-1pt]
  {\footnotesize\color{omuted}relational nachvollziehbar}
};

\draw[flow] (relevant) -- node[gate] {
  \textbf{Capability: Traceability}\\[-1pt]
  {\color{omuted}Intervention und Measure modellieren;}\\[-1pt]
  {\color{omuted}relationale Vollständigkeit prüfen}
} (traceable);

\node[state, below=1.72cm of traceable] (ready) {
  \textbf{Evidenzbereiter Wirkungstrace}\\[-1pt]
  {\footnotesize\color{omuted}ex ante geprüfter Nachweisentwurf}
};

\draw[flow] (traceable) -- node[gate] {
  \textbf{Capability: Evidenzbereitschaft}\\[-1pt]
  {\color{omuted}KPI-Definition, Evidenzplan, Baseline}\\[-1pt]
  {\color{omuted}und geplanten Beginn prüfen}
} (ready);
...
\node[final state, below=2.32cm of boundary] (assessed) {
  ...
  {\footnotesize\color{omuted}Effekt und Ziel getrennt bewertet}
};

\draw[flow] (ready) -- (boundary) -- node[gate, pos=0.72] {
  \textbf{Capability: Evidenzbewertung}\\[-1pt]
  {\color{omuted}Intervention durchführen und Folgebeobachtung erheben;}\\[-1pt]
  {\color{omuted}gegen Effekt- und Zielkriterium bewerten}
} (assessed);
