...
-- | Retain one complete QualificationSubjects failure at the command
-- boundary. Common failures delegate immediately to their canonical command
-- representation.
qualificationSubjectsCommandError ::
     QualificationSubjectsFailure -> CommandError
qualificationSubjectsCommandError failure =
  foldQualificationSubjectsFailure
    commonCommandError
    (const (QualificationSubjectsCommandError failure))
    (const (QualificationSubjectsCommandError failure))
    failure

-- | Retain one complete Trace failure at the command boundary. Common
-- failures delegate immediately to their canonical command representation.
traceCommandError :: TraceFailure -> CommandError
traceCommandError failure =
  foldTraceFailure
    commonCommandError
    (const (TraceCommandError failure))
    failure
...
commandErrorCode commandError =
  case commandError of
    ...
    QualificationSubjectsCommandError failure ->
      foldQualificationSubjectsFailure
        commonFailureCode
        (const "qualification-subjects.supplemental-input")
        (const "qualification-subjects.owner-contract")
        failure
    TraceCommandError failure ->
      foldTraceFailure commonFailureCode (const "trace.owner-contract") failure
...
foldCommandError ::
  ...
  -> CommandError
  -> result
foldCommandError argument process preparation validate qualify readiness assess qualificationSubjects trace commandError =
  case commandError of
    ...
    QualificationSubjectsCommandError failure -> qualificationSubjects failure
    TraceCommandError failure -> trace failure
